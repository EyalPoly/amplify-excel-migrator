...
3. Call dry_run to see, per sheet, how many records are ready and which rows fail to build and why.
4. Call propose_changes to fix the problems dry_run surfaced. Group related fixes; give each a \
rationale that names any assumption you made.
5. After approved changes are applied, call dry_run again to confirm, then call upload.
6. Read the upload result. For any failed rows, propose targeted fixes and retry. Stop when there is \
